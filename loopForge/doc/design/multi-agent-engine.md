...
**运行时逻辑规则**：在硬限制之外，**父子上下文、生命周期、异步执行、嵌套 spawn 授权、子内 LLM 流式策略、子工具集策略** 等**引擎内契约**以 **`doc/design/spawn-runtime-rules.md`** 为准。要点摘要：**(1)** 子默认不继承父全量历史，**允许** 父下传**精简**短期记忆；**(2)** **父 Run 终态** 时**所有** 子**必须**退出；**(3)** 子**单次**结束后是**即回收**还是**可再被父唤醒**由**父在 spawn 时**决定；**(4)** 子**推进** 在**独立**协程/异步执行，**不** 因等待子**整次** 子 Run 而**阻塞**父主路径；**(5)** 嵌套 spawn **默认禁止**：子级是否能再次 spawn 由父级在 `SpawnSpec` 中**显式授权**（`AllowChildSpawn`），**逐级**显式、**不传递不继承**，与 `MaxDepth` 取最严；**(6)** 子 **Run** 内**调模型** **须** **非流式**（`non-streaming`）以降低**网络** 开销，**与** 父/根路径的流式**配置** 解耦；**(7)** 子工具集**与父同源**，父级以 `tool_white_list` + `tool_block_list` 双名单收敛（**白先黑后**）；子 ReAct **中间** 步骤对父**不可见**，子级**只回最终** `SpawnResult`，**但**附带 `ToolInvocations` 摘要供父 Agent 向外发送工具调用报文。
